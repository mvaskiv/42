...
static unsigned long long		encrypt_decrypt2(unsigned long long s_blk,
										unsigned long long *subkeys, t_opt *opt)
{
	...
	if (opt->encrypt)
		s_blk = encode(s_blk / 4294967296, s_blk % 4294967296, subkeys, 0);
	else
...
static void						encrypt_decrypt(unsigned char *text, t_opt *opt)
{
	...
	i = 0;
	get_permutate_subkeys(opt, subkeys);
